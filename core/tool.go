...
// (retry, pick another file, try a different command) rather than aborting the whole run — an agent's
// core value is recovering from errors, so a single failing tool must not kill the session:
//   - IsError=true (with a nil error return): expected business error (file not found, command failed,
//     permission denied). Preferred form; all built-in tools use this.
//   - Exec returns error != nil: unexpected system/execution error (e.g. an MCP transport failure or a
//     tool panic downgraded by the loop). It is surfaced to the model as an "Error: ..." tool result
//     and marked ToolIsError; the loop continues.
//
